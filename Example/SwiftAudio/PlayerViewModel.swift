...
extension PlayerView {
    final class ViewModel: ObservableObject {
        ...
        @Published var cachedFilePath: String? = nil
        ...
        init() {
            ...
            controller.player.event.currentItem.addListener(self, handleCurrentItemChanged)
            controller.player.event.trackFullyLoaded.addListener(self, handleTrackFullyLoaded)
        }
        ...
        private func render() {
            DispatchQueue.main.async { [weak self] in
                ...
                let trackId = controller.player.currentItem?.getSourceUrl()
        ...
        func handleTrackFullyLoaded(data: AudioPlayer.TrackFullyLoadedEventData) {
            DispatchQueue.main.async { [weak self] in
                guard let self else { return }
                let currentId = controller.player.currentItem?.getSourceUrl()
                if data.trackId == nil || data.trackId == currentId {
                    cachedFilePath = data.filePath
                }
            }
        }

        func handleCurrentItemChanged(data: AudioPlayer.CurrentItemEventData) {
            render()
        }
