...
protocol AVPlayerItemObserverDelegate: AnyObject {
    ...
    /**
     Called when the observed item has fully loaded its media data.
     */
    ...
    override func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey : Any]?, context: UnsafeMutableRawPointer?) {
        ...
        case AVPlayerItemKeyPath.loadedTimeRanges:
            if let ranges = change?[.newKey] as? [NSValue] {
                if let duration = ranges.first?.timeRangeValue.duration {
                    delegate?.item(didUpdateDuration: duration.seconds)
                }
                if let item = observingItem,
                   let timeRange = ranges.first?.timeRangeValue,
                   CMTimeGetSeconds(timeRange.start) == 0,
                   CMTimeGetSeconds(CMTimeAdd(timeRange.start, timeRange.duration)) >= CMTimeGetSeconds(item.duration) {
                    delegate?.itemDidFullyLoad()
                }
            }
